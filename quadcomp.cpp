...
const char *ldnames[] = {"Quadruple", "Double Extended",
                         "Double", "Bad Compiler Error",
                         "Unknwon"};
...
std::array<calctype, 2> naiveQuadraticRoots(
    const intype coeffs[3]) {
  calctype disc =
...
std::array<fptype, 2> kahanQuadraticRoots(
    const fptype coeffs[3]) {
  fptype disc =
      coeffs[1] * coeffs[1] - 4 * coeffs[0] * coeffs[2];
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < numTrials; i++) {
    float coeffs[3];
    for (int j = 0; j < 3; j++) coeffs[j] = rgenf(engine);
    coeffs[0] = 1.0;
    coeffs[1] = 1.0;
    coeffs[2] = -12.0;
    auto correctRoots =
        naiveQuadraticRoots<float, long double>(coeffs);
    for (int j = 0; j < 2; j++) {
      float ev =
          evalPolynomial<float, 3>(coeffs, correctRoots[j]);
      printf("Root %d: %Le -> %e\n", j, correctRoots[j],
             ev);
    }
  }
